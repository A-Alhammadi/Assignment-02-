...
void modifyInt(string File, int count) {
	...
	try {
		...
		cin >> ind;
		if (ind > 99 || ind < 0) {
			throw logic_error("ERROR! Please enter a number in the index range (0-99)");
		}

		//and the new integer then the old integer at the index given is changed to the new one at the index number provided
		cout << "Enter the new value you want to change to: " << endl;
		cin >> newint;
		if (cin.fail())
		{
			throw logic_error("ERROR! Please enter an INTEGER to be changed");
		}
		arr[ind] = newint;
		cout << "The integer at index " << ind << " has been changed to : " << arr[ind] << endl;
	}
	catch (...){
		cout << "ERROR! Please enter an appropriate input.";
	}
}
